...
namespace Suprifattus.Util.Win32
{
	/// <summary>
	/// Contém comandos do Shell do Windows que não estão expostos pela API do .NET.
	/// </summary>
	public static class ShellApi
	{
		/// <summary>
		/// Adiciona um documento à lista dos documentos recentes do Windows
		/// </summary>
		/// <param name="path">O caminho completo do documento</param>
		public static void RecentDocsAdd(string path)
		{
			try
			{
				NativeMethods.SHAddToRecentDocs(NativeMethods.SHAddToRecentDocsFlags.Path, path);
			}
			catch (Exception ex)
			{
				throw new ShellApiException(ex);
			}
		}
		...
		/// Executa um comando.
		/// </summary>
		/// <param name="parent">A janela que será "dona" do comando</param>
		/// <param name="verb">O comando que será executado (ex: <c>open</c>)</param>
		/// <param name="file">O arquivo que será executado (caminho completo)</param>
		public static void ShellExecute(Form parent, string verb, string file)
		{
			if (parent == null)
			...
			try
			{
				parent.Cursor = Cursors.AppStarting;
				try
				{
					NativeMethods.ShellExecute(parent.Handle, verb, file, null, null, 0);
				}
				catch (Exception ex)
				{
					throw new ShellApiException(ex);
		...
		/// <summary>
		/// Adiciona uma associação de arquivo
		/// </summary>
		/// <param name="ext">A extensão do arquivo, iniciando com . (ponto)</param>
		/// <param name="className">O nome da classe. Pode ser qualquer nome ou um GUID</param>
		/// <param name="commandLine">A linha de comando que será executada, incluindo variáveis expansíveis, como %L</param>
		/// <param name="icon">O arquivo que contém o ícone do documento, ou <c>null</c> para utilizar um ícone gerado automaticamente</param>
		public static void AddFileAssociation(string ext, string className, string commandLine, string icon)
		{
			try
			{
				// ReSharper disable PossibleNullReferenceException

				// cria classe relacionada à extensão
				...
				{
					// associa o executável atual à abertura dos arquivos com a extensão
					using (RegistryKey key = classKey.CreateSubKey(@"shell\open\command"))
						key.SetValue(null, commandLine);

					if (icon != null)
					{
						// atribui o ícone de documento
						using (RegistryKey key = classKey.CreateSubKey(@"DefaultIcon"))
							key.SetValue(null, icon);
					}
				}

				NativeMethods.SHChangeNotify(
					NativeMethods.ShellChangeNotificationEvents.SHCNE_ASSOCCHANGED,
					NativeMethods.ShellChangeNotificationFlags.SHCNF_IDLIST,
		...
		internal class NativeMethods
		{
			...
			private delegate Int32 BrowseCallbackProc(IntPtr hwnd, UInt32 uMsg, Int32 lParam, Int32 lpData);
			...
			public struct STRRET
			{
				...
				/// </summary>
				[FieldOffset(0)] private UInt32 uType;

				/// <summary>
				/// must be freed by caller of GetDisplayNameOf
				/// </summary>
				[FieldOffset(4)] private IntPtr pOleStr;

				/// <summary>
				/// NOT USED
				/// </summary>
				[FieldOffset(4)] private IntPtr pStr;

				/// <summary>
				/// Offset into SHITEMID
				/// </summary>
				[FieldOffset(4)] private UInt32 uOffset;

				/// <summary>
				/// Buffer to fill in (ANSI)
				/// </summary>
				[FieldOffset(4)] private IntPtr cStr;
			}
			...
			{
				private UInt32 cbSize; // Size of the structure, in bytes. 
				private UInt32 fMask; // Array of flags that indicate the content and validity of the 
				// other structure members.
				private IntPtr hwnd; // Window handle to any message boxes that the system might produce
				// while executing this function. 
				[MarshalAs(UnmanagedType.LPWStr)] private String lpVerb; // String, referred to as a verb, that specifies the action to 
				// be performed. 
				[MarshalAs(UnmanagedType.LPWStr)] private String lpFile; // Address of a null-terminated string that specifies the name of 
				// the file or object on which ShellExecuteEx will perform the 
				// action specified by the lpVerb parameter.
				[MarshalAs(UnmanagedType.LPWStr)] private String lpParameters; // Address of a null-terminated string that contains the 
				// application parameters.
				[MarshalAs(UnmanagedType.LPWStr)] private String lpDirectory; // Address of a null-terminated string that specifies the name of 
				// the working directory. 
				private Int32 nShow; // Flags that specify how an application is to be shown when it 
				// is opened.
				private IntPtr hInstApp; // If the function succeeds, it sets this member to a value 
				// greater than 32.
				private IntPtr lpIDList; // Address of an ITEMIDLIST structure to contain an item identifier
				// list uniquely identifying the file to execute.
				[MarshalAs(UnmanagedType.LPWStr)] private String lpClass; // Address of a null-terminated string that specifies the name of 
				// a file class or a globally unique identifier (GUID). 
				private IntPtr hkeyClass; // Handle to the registry key for the file class.
				private UInt32 dwHotKey; // Hot key to associate with the application.
				private IntPtr hIconMonitor; // Handle to the icon for the file class. OR Handle to the monitor 
				// upon which the document is to be displayed. 
				private IntPtr hProcess; // Handle to the newly started application.
			}
			...
			{
				private IntPtr hwnd; // Window handle to the dialog box to display information about the 
				// status of the file operation. 
				private UInt32 wFunc; // Value that indicates which operation to perform.
				private IntPtr pFrom; // Address of a buffer to specify one or more source file names. 
				...
				private IntPtr pTo; // Address of a buffer to contain the name of the destination file or 
				...
				private UInt16 fFlags; // Flags that control the file operation. 
				private Int32 fAnyOperationsAborted; // Value that receives TRUE if the user aborted any file operations
				// before they were completed, or FALSE otherwise. 
				private IntPtr hNameMappings; // A handle to a name mapping object containing the old and new 
				// names of the renamed files. This member is used only if the 
				// fFlags member includes the FOF_WANTMAPPINGHANDLE flag.
				[MarshalAs(UnmanagedType.LPWStr)] private String lpszProgressTitle; // Address of a string to use as the title of a progress dialog box.
			...
			[Obsolete("Use System.Environment.GetFolderPath")]
			[DllImport("shell32.dll")]
			...
			/// <param name="lpOperation">Pointer to a null-terminated string, 
			/// referred to in this case as a verb, that specifies the action to be performed.</param>
			/// <param name="lpFile">Pointer to a null-terminated string that specifies the file 
			/// or object on which to execute the specified verb.</param>
			/// <param name="lpParameters">If the lpFile parameter specifies an executable file, 
			/// lpParameters is a pointer to a null-terminated string that specifies the parameters 
			/// to be passed to the application.</param>
			...
			public static extern void SHChangeNotify(
				ShellChangeNotificationEvents wEventId,
				ShellChangeNotificationFlags uFlags,
			...
			/// <param name="pv">A pointer to either a null-terminated string with the path and file name of the document, or a PIDL that identifies the document's file object. Set this parameter to NULL to clear all documents from the list.</param>
			[DllImport("shell32.dll")]
			...
			#region Extern Enum Declarations
			public enum CSIDL
			{
			...
			public enum SHAddToRecentDocsFlags : uint
			{
				Pidl = 0x001,
				Path = 0x002,
			}
	...
	#region ShellApiException
	/// <summary>
	/// Representa uma exceção lançada pela ShellApi.
	/// </summary>
	[Serializable]
	public class ShellApiException : ApplicationException
	{
		/// <summary>
		/// Cria uma nova exceção da ShellApi
		/// </summary>
		public ShellApiException()
		{
		}
		...
		/// Cria uma nova exceção da ShellApi
		/// </summary>
		protected ShellApiException(SerializationInfo info, StreamingContext context)
			: base(info, context)
		{
		}
		...
		/// <param name="innerException">A exceção causadora</param>
		public ShellApiException(string message, Exception innerException)
		...
		/// </summary>
		/// <param name="message">A mensagem da exceção</param>
		public ShellApiException(string message)
			: base(message)
		{
		}
		...
		/// </summary>
		/// <param name="innerException">A exceção causadora</param>
		public ShellApiException(Exception innerException)
